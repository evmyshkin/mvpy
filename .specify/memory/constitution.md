...
- Не использовать `joinedload`
- Не использовать `selectinload`
- Использовать явные `join`-операции возврата данных из связанных таблиц.

**Обоснование**: С предлложенными практиками повышается читаемость кода, наглядность и быстродействие. Не используются дополнительные механики ORM и все взаимодействие с БД работает за счет механизмов самой СУБД.
...
- В контроллере допускается только прямой вызов метода CRUD.
- В контроллере также допускается вызов метода сервиса, если не подходит вызов CRUD.
- Другой логики внутри контроллеров не допускается.
- Вся бизнес логика реализуется внутри сервисов, а контроллер может вызывать методы сервиса.
- Инициализация сервиса не должна происходить внутри контроллера.

### XX. Среда разработки

- Проверяй наличие инструментов, доступных тебе
- Используй MCP от IDE, если оно предосталвено
- Используй MCP Docker, если он предоставлен
- Сообщай о недоступности тех или иных MCP пользователю, чтобы он мог среагировать и облегчить твою работу включением MCP.
