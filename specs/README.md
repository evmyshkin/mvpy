# Директория спромтами для `spec-kit`

Папка `spec_input` содержит шаблоны текстовых файлов для удобного промтинга разных вызовов функций `spec-kit`.

## Флоу

Инструкция, как по шагам с помощью `spec-kit` был запущен этот проект с помощью ИИ-агента с нуля. 
...
- Флоу ниже описан подробнее в моей [статье на Хабре](https://habr.com/ru/articles/993774/) 
- Для команд, требующих аргументов с деталями ТЗ, можно использовать ChatGPT.
- Рекомендую перечитывать все бизнесовое, что генерит нейронка. 
- Не нужно раздувать ТЗ за скоуп, который требует заказчик.
- Из внутренностей `spec-kit` специально выкинуты несколько артефактов, которые на моем опыте дают мало бенефитов:
  - `checklists.md`: это обязательный артефакт из коробки при вызове команды `/speckit.specify`. Создает файл `checklists.md` внутри директории фичи. Убрал, так создание файла мешает в одно касание вычитать и забыть о бизнес-требованиях. Плюс раздувает контекст. Нейронка, как правило, сама там просто галочки ставит и все. Лишний документ.
  - `quickstart.md`: это обязательный артефакт вызова команды `/speckit.plan`. Непонятно, для кого он нужен. У нейронки и так есть наш проект, спека фичи и `constitution.md`, чтобы все знать, что нужно. Чтобы не плодить дубли, урано.
...
## 3. `/speckit.plan` – Архитектура и имплементация
...
## 5. `/speckit.analyze` – Поиск проблем
