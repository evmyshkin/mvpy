...
### История пользователя 2 - Администратор по умолчанию (Приоритет: P2)

**Как администратор системы**, я хочу предварительно сконфигурированную административную учётную запись, доступную сразу после развертывания системы, чтобы я мог управлять системой через API без необходимости ручной настройки.

**Почему этот приоритет**: Необходим для развёртывания системы и первоначального управления. Без администратора по умолчанию системе потребуется ручное вмешательство в базу данных для создания первого пользователя с правами администратора, что усложняет развёртывание и первоначальную настройку.

**Независимая проверка**: Может быть протестирована развёртыванием нового экземпляра системы и попыткой аутентификации с учётными данными администратора по умолчанию. Доставляет немедленный административный доступ к системе.

**Приемочные сценарии**:

1. **Дано** система только что развернута, **Когда** миграции базы данных завершены, **Тогда** существует учётная запись администратора
2. **Дано** администратор по умолчанию существует, **Когда** я аутентифицируюсь с учётными данными по умолчанию, **Тогда** я получаю валидный токен доступа
3. **Дано** администратор по умолчанию существует, **Когда** я запрашиваю детали пользователя, **Тогда** пользователь имеет назначенную роль администратора
...
- **FR-003**: Система ДОЛЖНА назначать роль "администратор" пользователю-администратору по умолчанию, создаваемому во время развертывания
